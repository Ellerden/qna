...
feature 'User can sign in via social networks', %q{
  ...
  describe 'User signs in via Facebook' do
    context 'New user signs in using Facebook for the 1st time' do
      ...
      context 'Email confirmation needed'
        ...
        scenario 'Email confirmed and user tries to log in via Facebook' do
          ...
          expect(page).to have_content("Your account was succesfully verified. Now you can log in via Facebook")

          # LOG IN WITH Facebook after confirming
          click_on 'Sign in with Facebook'
    ...
    context 'User somehow already exists' do
      ...
      scenario 'Already authorized via FB user signs using Facebook for the 2nd time' do
      ...
      scenario 'Existing user signs in using Facebook for the first time' do
        auth = mock_auth_hash(:facebook)
        user.update!(email: auth.info.email)
        visit new_user_session_path
        expect(page).to have_content('Sign in with Facebook')
